...
final class NetworkService: ObservableObject {
    ...
    func sendMediaFile(
        ...
        session.dataTask(with: request) { data, response, error in
            ...
            do {
                let decoded = try self.jsonDecoder.decode(MessageResponse.self, from: data)
                guard decoded.success, let messageData = decoded.message else {
                    let errText = decoded.error ?? "Failed to send media"
                    DispatchQueue.main.async {
                ...
                let message = self.mapMessage(messageData)
    ...
    func sendMediaFilesGroup(
        ...
        guard let url = makeURL("/send-media-group") else {
            completion(.failure(NSError(domain: "NetworkService", code: -70, userInfo: [
                NSLocalizedDescriptionKey: "Invalid send-media-group URL"
            ])))
            return
        }
        ...
        if let caption, !caption.isEmpty {
            appendField(name: "text", value: caption)
        }

        let durationValues = durations.map { $0 ?? 0 }
        if let durationsData = try? JSONEncoder().encode(durationValues),
           let durationsString = String(data: durationsData, encoding: .utf8) {
            appendField(name: "durations", value: durationsString)
        }

        var appendedFilesCount = 0
        ...
        if appendedFilesCount == 0 {
            completion(.failure(NSError(domain: "NetworkService", code: -74, userInfo: [
                NSLocalizedDescriptionKey: "No files could be appended to multipart body"
            ])))
            return
        }
        ...
        session.dataTask(with: request) { data, response, error in
            ...
            guard let httpResponse = response as? HTTPURLResponse,
                  let data = data else {
                DispatchQueue.main.async {
                    completion(.failure(NSError(domain: "NetworkService", code: -71, userInfo: [
                        NSLocalizedDescriptionKey: "Invalid server response"
                    ])))
                }
                return
            ...
            do {
                let decoded = try self.jsonDecoder.decode(MessageResponse.self, from: data)
                guard decoded.success, let messageData = decoded.message else {
                    let errText = decoded.error ?? "Failed to send media group"
                    DispatchQueue.main.async {
                        completion(.failure(NSError(domain: "NetworkService", code: -72, userInfo: [
                            NSLocalizedDescriptionKey: errText
                        ])))
                    }
                    return
                ...
                let raw = String(data: data, encoding: .utf8) ?? "Unable to decode response"
                DispatchQueue.main.async {
                    completion(.failure(NSError(domain: "NetworkService", code: -73, userInfo: [
                        NSLocalizedDescriptionKey: "Decoding error: \(error.localizedDescription)\nResponse: \(raw)"
                    ])))
                }
            }
