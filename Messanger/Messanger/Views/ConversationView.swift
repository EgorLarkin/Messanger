...
struct ConversationView: View {
            ...
            ScrollViewReader { proxy in
                ScrollView {
                    LazyVStack(spacing: 8) {
                        // ✅ Сообщения отображаются по порядку: старые сверху, новые снизу
    ...
    // ✅ Прокрутка к ПОСЛЕДНЕМУ сообщению (вниз)
    ...
    // ✅ Загрузка сообщений: старые сверху, новые снизу
    private func loadMessages() {
        ...
        networkService.fetchHistory(user1: current.username, user2: chatUser.username) { result in
            ...
            case .success(let fetched):
                // ✅ Сортировка: от СТАРЫХ к НОВЫМ (возрастание)
