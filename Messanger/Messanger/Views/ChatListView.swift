...
struct ChatListView: View {
    ...
    var body: some View {
        NavigationView {
            ...
            .background(
                NavigationLink(
                    destination: ConversationView(chatUser: selectedUserForNavigation ?? User(id: "", username: "", name: "")),
                ...
                Section(header: Text("Ваши чаты")) {
                    ForEach(conversations) { conv in
                        NavigationLink(destination: ConversationView(chatUser: conv.user)) {
                            HStack {
                                Image(systemName: "person.circle.fill").font(.system(size: 45)).foregroundColor(.blue)
