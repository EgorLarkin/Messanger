...
struct ConversationView: View {
    ...
    private func sendMediaItems(_ items: [PendingMediaItem], caption: String) {
        ...
        let imageItems = items.filter { isImageItem($0) }
        let videoItems = items.filter { isVideoItem($0) }

        print("🧪 Pending items count:", items.count)
        print("🧪 Image items:", imageItems.count)
        print("🧪 Video items:", videoItems.count)
        print("🧪 Raw item types:", items.map { $0.type.rawValue })

        if imageItems.count == items.count {
            sendGroupAttachments(imageItems, caption: normalizedCaption, groupType: .image) { _ in
                finishSendingBatch()
            }
            return
        }

        if videoItems.count == items.count {
            sendVideoGroup(videoItems, caption: normalizedCaption) { _ in
                finishSendingBatch()
            }
            return
        }

        if items.count > 1, imageItems.count + videoItems.count == items.count {
            sendMixedMediaGroup(items, caption: normalizedCaption) { _ in
                finishSendingBatch()
            }
            return
        }
    ...
    private func finishSendingBatch() {
        DispatchQueue.main.async {
    ...
    private func isImageItem(_ item: PendingMediaItem) -> Bool {
        item.type == .image || item.image != nil
    }

    private func isVideoItem(_ item: PendingMediaItem) -> Bool {
        item.type == .video || item.type == .videoNote || item.videoURL != nil
    }
    ...
    private func sendMixedMediaGroup(
        _ items: [PendingMediaItem],
        caption: String,
        completion: @escaping (Bool) -> Void
    ) {
        networkService.sendMixedMediaGroup(
            to: chatUser.username,
            items: items,
            caption: caption
        ) { result in
            ...
            case .failure(let error):
                print("❌ Failed to send mixed media group: \(error.localizedDescription)")
                DispatchQueue.main.async {
                    completion(false)
                }
            }
