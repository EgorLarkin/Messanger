...
struct MessageBubble: View {
    ...
    private var myMessageView: some View {
        VStack(alignment: .trailing, spacing: 4) {
            ...
            if message.type != .text,
               !message.formattedDuration.isEmpty,
               message.type != .video,
               message.type != .mixed {
        ...
        VStack(alignment: .leading, spacing: 6) {
            LazyVGrid(
                ...
                ForEach(Array(imageAttachments.enumerated()), id: \.element.id) { index, attachment in
                    ProtectedImageThumbnailView(
                        attachment: attachment,
                        width: 100,
                        height: 100,
                        fileName: attachment.fileName ?? "Фото \(index + 1)"
                    )
                }

                ForEach(Array(videoAttachments.enumerated()), id: \.element.id) { _, attachment in
                    if let url = attachment.videoURL {
                        NavigationLink(destination: VideoPlayerView(url: url)) {
                            VideoThumbnailView(
                                url: url,
                                durationText: formatDuration(attachment.duration),
                                width: 100,
                                height: 100
                            )
                        }
    ...
    private var imageContent: some View {
        let attachments = message.imageAttachments
            ...
            if let attachment = attachments.first {
                VStack(alignment: .leading, spacing: 0) {
                    ProtectedImageThumbnailView(
                        ...
                        width: 220,
                        height: 220,
                        fileName: message.fileName ?? attachment.fileName ?? "Фото"
                    )
    ...
    private var fallbackVideoGridItem: some View {
        VStack(spacing: 4) {
            Image(systemName: "video")
                .font(.system(size: 28))
                .foregroundColor(.white.opacity(0.7))
    ...
    private var fileContent: some View {
        HStack(spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                Text(message.fileName ?? message.firstAttachment?.fileName ?? "Файл")
                    .font(.headline)
                    .lineLimit(1)
                ...
                if let size = message.fileSize {
                    Text(formatFileSize(size))
                        .font(.caption)
                        .foregroundColor(.gray)
                } else if let size = message.firstAttachment?.fileSize {
                    Text(formatFileSize(size))
                        .font(.caption)
                        .foregroundColor(.gray)
                }
    ...
    private var videoContent: some View {
        let videos = message.videoAttachments
        ...
        if videos.count <= 1 {
            ...
            VStack(alignment: .leading, spacing: 6) {
                LazyVGrid(
                    ...
                    ForEach(Array(videos.enumerated()), id: \.element.id) { _, attachment in
                        if let url = attachment.videoURL {
                            NavigationLink(destination: VideoPlayerView(url: url)) {
                                VideoThumbnailView(
                                    url: url,
...
struct ProtectedImageThumbnailView: View {
    ...
    var body: some View {
        Group {
            if let uiImage = uiImage {
                let displayImage = Image(uiImage: uiImage)
    ...
    private func loadImage() async {
        ...
        if let localImage = decodeBase64Image() {
            await MainActor.run {
        ...
        guard let url = attachment.remoteURL else { return }
        let loaded = await ProtectedImageLoader.loadImage(from: url)
    ...
    private func decodeBase64Image() -> UIImage? {
        let source = attachment.resolvedMediaURLString
...
enum ProtectedImageLoader {
    ...
    static func loadImage(from url: URL) async -> UIImage? {
        ...
        guard let localURL = localURL else {
            print("❌ Failed to prepare local image file: \(url)")
            return nil
        }

        guard let data = try? Data(contentsOf: localURL),
              let image = UIImage(data: data) else {
            print("❌ Failed to decode image from local file: \(localURL)")
            return nil
        }
    ...
    private static func prepareLocalURL(from url: URL) async -> URL? {
        ...
        guard let token = AuthManager.shared.token, !token.isEmpty else {
            print("❌ Failed to download remote image: missing auth token")
            return nil
        }
        ...
        do {
            ...
            guard let httpResponse = response as? HTTPURLResponse else {
                print("❌ Failed to download remote image: invalid response")
                return nil
            }

            guard (200...299).contains(httpResponse.statusCode) else {
                let responseText = String(data: data, encoding: .utf8) ?? "No response body"
                print("❌ Failed to download remote image: status=\(httpResponse.statusCode), body=\(responseText)")
                return nil
            }
