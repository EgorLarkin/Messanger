...
struct ChatListView: View {
            ...
            if !conversations.isEmpty {
                Section(header: Text("Ваши чаты")) {
                    ForEach(conversations) { conv in
                        NavigationLink(
                            destination:
                                ConversationView(chatUser: conv.user)
                                .onAppear {
                                    markConversationAsRead(conv.chatWith)
                                }
                        ) {
                            HStack {
    ...
    private func markConversationAsRead(_ username: String) {
        guard let index = conversations.firstIndex(where: { $0.chatWith == username }) else { return }
