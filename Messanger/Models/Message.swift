...
struct MessageAttachment: Codable, Equatable, Identifiable {
    ...
    var detectedType: MediaType {
        ...
        if source.hasPrefix("http://") || source.hasPrefix("https://") {
            if name.hasSuffix(".jpg") || name.hasSuffix(".jpeg") || name.hasSuffix(".png") || name.hasSuffix(".webp") || name.hasSuffix(".heic") {
                return .image
            }
...
struct Message: Codable, Identifiable, Equatable {
    ...
    var type: MediaType {
        if let mapped = MediaType(rawValue: mediaType) {
            return mapped
        }
    ...
    var displayText: String {
        ...
        case .mixed:
            let imagesCount = imageAttachments.count
            let videosCount = videoAttachments.count
            let total = (attachments ?? []).count

            if !text.isEmpty { return text }
            if imagesCount > 0 && videosCount > 0 { return "Медиа: \(total)" }
            if imagesCount > 0 { return "Фотографии: \(imagesCount)" }
            if videosCount > 0 { return "Видео: \(videosCount)" }
            return "Вложения: \(total)"
