...
const conversationsDB = {}; // Активные чаты
...
app.post('/auth/register', async (req, res) => {
    ...
    try {
        ...
        usersDB[username] = { id: userId, username, name, passwordHash, createdAt: new Date().toISOString() };
        const token = jwt.sign({ username }, JWT_SECRET, { expiresIn: '24h' });
        tokensDB[token] = username;
        console.log(`✅ [REGISTER] ${username} (${userId})`);
        res.status(201).json({ success: true, token, user: { id: userId, username, name } });
    } catch (error) {
...
app.get('/auth/verify', (req, res) => {
    ...
    const user = usersDB[username];
    res.json({ valid: true, user: { id: user.id, username: user.username, name: user.name } });
});
...
app.get('/users', (req, res) => {
    ...
    const userList = Object.values(usersDB).map(u => ({ id: u.id, username: u.username, name: u.name }));
...
app.put('/profile', (req, res) => {
    ...
    res.json({ success: true, user: { id: usersDB[username].id, username: usersDB[username].username, name: usersDB[username].name } });
});
...
app.get('/conversations', (req, res) => {
    ...
    const enriched = conversations.map(conv => ({
        ...conv,
        user: usersDB[conv.chatWith] ? { id: usersDB[conv.chatWith].id, username: usersDB[conv.chatWith].username, name: usersDB[conv.chatWith].name } : null
...
app.post('/send', (req, res) => {
    ...
    // ✅ ОБНОВЛЕНИЕ ЧАТОВ ДЛЯ ОБОИХ ПОЛЬЗОВАТЕЛЕЙ
